...
# The console is minimized at startup (not hidden), and closed together with
# the GUI when the optimizer window is closed.
# -----------------------------------------
...
# Minimize the console now (nCmdShow = 6 => SW_MINIMIZE)
...
try {
    ...
    if ($Script:ConsoleHwnd -ne [IntPtr]::Zero) {
        [Native.Win32Console]::ShowWindow($Script:ConsoleHwnd, 6) | Out-Null
    }
} catch { }
...
# Console was already minimized at startup. Nothing to hide here.

$Window.ShowDialog() | Out-Null

# -----------------------------------------
# GUI CLOSED  --  close the owning console window too, so no PowerShell
# window is left behind after the optimizer is closed.
# Safety: only close the console if it belongs to THIS PowerShell process
# (the normal case when launched via -File or 'irm | iex'). This avoids
# accidentally killing an unrelated parent terminal.
# -----------------------------------------
try {
    if ($Script:ConsoleHwnd -ne $null -and $Script:ConsoleHwnd -ne [IntPtr]::Zero) {
        [uint32]$procId = 0
        [Native.Win32Console]::GetWindowThreadProcessId($Script:ConsoleHwnd, [ref]$procId) | Out-Null
        if ($procId -ne 0 -and $procId -eq $PID) {
            # Console is owned by this process -> safe to close it.
            Stop-Process -Id $procId -Force -ErrorAction SilentlyContinue
        } else {
            # Console belongs to a parent terminal (e.g. irm | iex). Just restore
            # it from minimized state so the user gets their prompt back.
            [Native.Win32Console]::ShowWindow($Script:ConsoleHwnd, 9) | Out-Null  # SW_RESTORE
        }
    }
} catch { }
